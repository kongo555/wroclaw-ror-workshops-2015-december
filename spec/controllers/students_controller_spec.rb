...
RSpec.describe StudentsController do
  ...
  describe 'PUT #update' do
    let!(:student) { create :student, first_name: 'John', last_name: 'Smith', birthdate: '2000-10-21' }
    let(:first_name) { 'Walter' }
    let(:last_name) { 'White' }
    let(:birthdate) { '1992-10-13' }
    let!(:params) do
      { id: student.id, student: { first_name: first_name, last_name: last_name, birthdate: birthdate } }
    end
    ...
    context 'success' do
      ...
      context 'updates student' do
        ...
        it { is_expected.to change{ student.reload.birthdate.strftime('%Y-%m-%d') }.to(birthdate) }
      end
    end
